// --------------------------------------- Project List
export const ProjectList = [
  {
    img: "/proj/hooraSP.jfif",
    title: "Hoora - Franchise Onboarding, Announcement etc.",
    description:
      "A Franchise Onboarding platform for the Hoora to quickly get new franchise Onboard with Announcement, Ticketing and Employee Hiring features.",
    tech_stack: ["MongoDB", "Express", "ReactJS", "NodeJS"],
    github_url: "https://github.com/shubhanshusahu/hoora",
    demo_url: "https://youknowtheending.net/",
    Show_demo_btn: false
  },
  {
    img: "/renr-responsive.png",
    title: "Menarini- Medicine Enquiry System",
    description:
      "A Web app for Patients/Medical Representatives to Query Information about Pharmaceuticals",
    tech_stack: ["Angular"],
    github_url: "https://github.com/shubhanshusahu/menariniMIE",
    demo_url: "https://renr-bcit.web.app/",
  },
  {
    img: "/proj/1ps.png",
    title: "Playmersiv-Sports/Stock footage/Ecommerce",
    description:
      "An app where different clients can create/ design their own site using the CMS Strapi, After publishing, their site will be available under http://playmersiv/ domain",
    tech_stack: ["Gatsby", "ReactJS", "Strapi"],
    github_url: "https://github.com/shubhanshusahu/playmersivProd",
    demo_url: "https://playmersiv.live/",
    Show_demo_btn: true
  },

];

// --------------------------------------- Skills

export const stackList = [
  {
    img: "https://raw.githubusercontent.com/devicons/devicon/master/icons/react/react-original.svg",
    name: "ReactJS",
  },
  {
    img: "https://logowik.com/content/uploads/images/179_angular.jpg",
    name: "Angular",
  },
  {
    img: "https://raw.githubusercontent.com/devicons/devicon/master/icons/javascript/javascript-original.svg",
    name: "JavaScript",
  },
  {
    img: "https://raw.githubusercontent.com/devicons/devicon/master/icons/typescript/typescript-original.svg",
    name: "Typescript",
  },
  {
    img: "https://raw.githubusercontent.com/devicons/devicon/master/icons/react/react-original.svg",
    name: "React Native",
  },
  {
    img: "https://raw.githubusercontent.com/devicons/devicon/master/icons/html5/html5-original.svg",
    name: "HTML",
  },
  {
    img: "https://raw.githubusercontent.com/devicons/devicon/master/icons/css3/css3-original.svg",
    name: "CSS",
  },
 
  {
    img: "https://raw.githubusercontent.com/devicons/devicon/master/icons/git/git-original.svg",
    name: "Git",
  },
  {
    img: "data:image/png;base64,",
    name: "Gatsby",
  },
  {
    img: "https://assets.super.so/e7c0f16c-8bd3-4c76-8075-4c86f986e1b2/uploads/favicon/9c68ae10-0a8a-4e3f-9084-3625b19df9cb.png",
    name: "Strapi",
  },
  {
    img: "https://raw.githubusercontent.com/devicons/devicon/master/icons/nodejs/nodejs-original.svg",
    name: "NodeJS",
  },
  {
    img: "https://raw.githubusercontent.com/devicons/devicon/master/icons/firebase/firebase-plain.svg",
    name: "Firebase",
  },
  {
    img: "https://raw.githubusercontent.com/devicons/devicon/master/icons/mongodb/mongodb-original.svg",
    name: "MongoDB",
  },
  {
    img: "https://raw.githubusercontent.com/devicons/devicon/master/icons/mysql/mysql-original.svg",
    name: "MySQL",
  },
  {
    img: "https://raw.githubusercontent.com/devicons/devicon/master/icons/bootstrap/bootstrap-original.svg",
    name: "Bootstrap",
  },
  {
    img: "https://raw.githubusercontent.com/devicons/devicon/master/icons/materialui/materialui-original.svg",
    name: "Material UI",
  },
  {
    img: "https://ih1.redbubble.net/image.438908244.6144/bg,f8f8f8-flat,750x,075,f-pad,750x1000,f8f8f8.u2.jpg",
    name: "Express",
  },

];
